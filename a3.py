...
def title_by_year(matches: List[str]) -> List[str]:
    ...
    """
    result = (list(map(get_title,
                 (filter(lambda movie:
                         int(matches[0])== get_year(movie), movie_db)))))
    return result

#helper funtions
def interval(lst):
    #expands the range of the year given.
	m = []
	lst = list(map(int,lst))
	diff = lst[len(lst)-1]- lst[0]
	i = 0
	while (i <= diff):
		ans = str(lst[0]+i)
		i += 1
...
Args:
        ...
        Returns:
            ...
            """
    result = []
    for n in interval(matches):
        result = result + title_by_year([n])
    return result
...
def title_by_director(matches: List[str]) -> List[str]:
    ...
    result = (list(map(get_title,
                 (filter(lambda movie:
                         matches[0] == get_director(movie), movie_db)))))
    return result
...
def num_by_year(matches: List[str]) -> List[str]:
    """returns the number of movies made in the passed in year
    ...
    Returns:
        a number of movies made in the passed in year
    """
    r = title_by_year(matches)
    return [len(r)]
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (["bye"], bye_action),
    (str.split("How many movies were made in _"), num_by_year)
]
...
if __name__ == "__main__":
    ...
    #assert sorted(
    #    search_pa_list(["How", "many", "movies", "were", "made", "in", "1988"])
    #) == [1], "failed search_pa_list test 4"
